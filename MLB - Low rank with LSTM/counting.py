...
class Counter(nn.Module):
    ...
    def filter_most_important(self, n, boxes, attention):
        ...
        # 检查 n 是否超过 attention 的第二个维度大小
        actual_n = min(n, attention.size(1))

        # 如果 n 超过了 attention 的第二个维度，则将 n 设置为这个维度的大小
        attention, idx = attention.topk(actual_n, dim=1, sorted=False)

        # 使用 gather 操作重新排列 boxes
